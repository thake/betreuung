        ...
        fun `test complete flow with replacement rules`() {
                ...
                val globalRule =
                        ReplacementRule(
                                ...
                                action =
                                        ReplacementAction(RuleField.PURPOSE, "Rent for {nachname}"),
                                isActive = true
                        )
                val rules = listOf(specificRule, globalRule)

                // 4. Mimic WorkScreen Logic: Apply Rules
                // Note: In real app, we load rules from DataManager. Here we pass them directly for
                // testing
                // the flow logic.
                val processedTx =
                ...
                assertTrue(
                        xmlContent.contains(
                                "<element id=\"BezeichnungPos3\">Online Order</element>"
                        ),
